...
const PostComments = ({ route }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://192.168.1.8:8080/api/publicaciones/${postId}/comentarios`);
    ...
    navigation.setOptions({
      headerRight: () => (
        <View style={{ flexDirection: 'row' }}>
          <TouchableOpacity onPress={() => navigation.navigate('CreateNewComment', { postId })}>
