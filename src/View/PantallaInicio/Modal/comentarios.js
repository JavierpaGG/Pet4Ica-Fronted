...
import { getComments, addComment } from '../../../Api/api'; // Importar las funciones de la API
...
const CommentsModal = ({ visible, postId, onClose }) => {
  ...
  const fetchComments = async () => {
    try {
      ...
      setError('Error al cargar los comentarios. Por favor, inténtelo de nuevo más tarde.');
    }
  };
...
const styles = StyleSheet.create({
  ...
  modal: {
    ...
    borderRadius: 10,
    width: '80%',
    maxHeight: '80%', // Limitar la altura del modal al 80% de la pantalla
  },
  title: {
    fontSize: 24,
    fontWeight: 'bold',
  ...
  commentsContainer: {
    maxHeight: '50%', // Limitar la altura de la lista de comentarios al 50% del modal
  },
});
