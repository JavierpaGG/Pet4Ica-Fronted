...
const MisSolicitudes = () => {
    ...
    const fetchSolicitudes = async () => {
      try {
        ...
        if (userData && userData.id) {
          const response = await fetch(`http:192.168.1.8:8080/api/adopciones/usuario/${userData.id}`);
  ...
  const renderSolicitud = ({ item }) => (
    <View style={styles.solicitudContainer}>
      ...
      <Text style={styles.nombreMascota}>{item.publicacion.mascota.nombre}</Text>
      <Image
        source={{ uri: getImageUrl(item.publicacion.mascota.imagenPath) }}
        style={styles.imagenMascota}
      />
      <Text style={styles.fechaSolicitud}>{new Date(item.fechaSolicitud).toLocaleDateString()}</Text>
